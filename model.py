...
def model(intput_x, input_y):
	...
	with tf.name_scope("paramsAtL1"):
		lyr1_w = tf.Variable(tf.compat.v1.random.truncated_normal(shape = [5, 5, 1, 64], dtype = tf.float32), name = "lyr1_w")
	...
	with tf.name_scope("paramsAtL2"):
		lyr2_w = tf.Variable(tf.compat.v1.random.truncated_normal(shape = [5, 5, 64, 128], dtype = tf.float32), name = "lyr2_w")
	...
	with tf.name_scope("paramsAtL3"):
		lyr3_w = tf.Variable(tf.compat.v1.random.truncated_normal(shape = [7 * 7 * 128, 864], dtype = tf.float32), name = "lyr3_w")
	...
	with tf.name_scope("paramsAtL4"):
		lyr4_w = tf.Variable(tf.compat.v1.random.truncated_normal(shape = [864, 432], dtype = tf.float32), name = "lyr4_w")
	...
	with tf.name_scope("paramsAtOptLayer"):
		opt_w = tf.Variable(tf.compat.v1.random.truncated_normal(shape = [432, N], dtype = tf.float32), name = "opt_w")
	...
	with tf.name_scope("optimizer"):
		optimizer = tf.compat.v1.train.AdamOptimizer(rate).minimize(crossEntropy)
